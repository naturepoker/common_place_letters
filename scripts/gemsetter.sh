#!/usr/bin/env bash
#script usage settings is as below:
# ./gemsetter.sh 1)filename.gmi 2)title of the post 3)description 4)tags separated by comma
...
gem_link=$(echo $input_post | sed 's/^/=> */g' | sed 's/*/gemini:\/\/gmi.si3t.ch\/echo\/gemlog_/g' | sed "s/$/ $gem_date/g" | sed "s/$/ $gem_desc/g")
cd gemset
